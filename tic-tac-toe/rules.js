...
function reset_play(){
	...
	document.getElementById("turn_info").innerText = "Game hasn't begun";
	var x = 0;
	for(x; x < 9; x++)
	{
...
function play() {
	...
	if((spot[0] == (/[A-C]/g)) && (spot[1] == (/[1-3]/g))); //Input now we need to check
	{
		let index_spot = table_ids.indexOf(spot);
		if(board_state[index_spot]!= -1)
		{
		...
		else if(turn == 1)
		{
			...
			document.getElementById("move_text_id").value = ""; // reset input field
		...
		//Check to see if they win this is first row
		if((board_state[0] != -1)&&(board_state[1]!=-1)&&(board_state[2]!=-1)&&(board_state[0] == board_state[1]) && (board_state[0] == board_state[2]))
		{
			if(board_state[0] = 1)
			{
				alert("X's have won. Now resetting");
				reset_play();
			}
			else
			{
				alert ("O's have won. Now Resetting");
				reset_play();
			}
		}
		//second row
